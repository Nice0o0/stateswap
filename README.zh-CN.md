...
- [ ] **分层切除**——全局 rank 切除会回基线（外科研究）；只在目标能力的
  层地址上切除某个子空间，也许能实现真外科
- [ ] **行为 vs 对话深度**——新鲜会话 + 深度可控合成历史的平衡态测量
  （混合行走协议会退化，见 phase-transition.md §8）
- [ ] **多人格对泛化**——双相变点相图是否普适？（需要第三个人格；炼丹厂能造）
- [ ] **长文撰写**——"The Geometry of RWKV Initial States"，大纲在
